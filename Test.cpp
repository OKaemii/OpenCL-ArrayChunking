...
int main(int argc, char** argv)
{
	...
	//std::vector<int> haloed_vec((4 + halo_size) * (4 + halo_size) * (4 + halo_size));
	//std::vector<int> result(haloed_vec.size());

	//// new dimensions with haloes
	//int haloed_width = (4 + halo_size);
	//int haloed_height = (4 + halo_size);
	//int haloed_depth = (4 + halo_size);
	...
	//			haloed_vec[index] = vec[original_index];
	//			original_index++;
	//		}
	//	}
	//}
	...
	//int x_chunk = haloed_width / chunkSlice;
	//int y_chunk = haloed_height / chunkSlice;
	//int z_chunk = haloed_depth / chunkSlice;

	//for (int chunked_z = 0; chunked_z  < haloed_depth; chunked_z  += z_chunk)
	//{
	//	for (int chunked_y = 0; chunked_y < haloed_height; chunked_y += y_chunk)
	//	{
	//		for (int chunked_x = 0; chunked_x < haloed_width; chunked_x += x_chunk)
	//		{
	//			printf("chunk main body array to send for device\n");
	//			std::vector<int> chunkedArrayHost(x_chunk * y_chunk * z_chunk);
	//			int host_index = 0;
	//			//printf("starting cords for chunk: (%d, %d, %d)\n", chunked_x, chunked_y, chunked_z);
	...
	//						int index = (x + chunked_x) + haloed_width * ((y + chunked_y) + haloed_height * (z + chunked_z));
	//						printf("trying to access: vec[%d/%d]\n", index, haloed_vec.size());
	//						chunkedArrayHost[host_index] = haloed_vec[index];
	//						host_index++;
	//					}
	//				}
	//			}
	...
	//			std::vector<int> chunkedArrayDevice(x_chunk * y_chunk * z_chunk);
	//			for (int id = 0; id < chunkedArrayDevice.size(); id++)
	//			{
	//				// find index of co-ordinates (x0, y0, z0)
	//				int loc_x = (id % x_chunk) + chunked_x;
	//				int loc_index = id / x_chunk;
	//				int loc_y = (loc_index % y_chunk) + chunked_y;
	//				int loc_z = (loc_index / y_chunk) + chunked_z;
	//				printf("DEVICE:: (%d, %d, %d) [%d, %d]\n", loc_x, loc_y, loc_z, id, chunkedArrayDevice.size());
	//				chunkedArrayDevice[id] = chunkedArrayHost[id];
	//			}
	...
	//						result[index] = chunkedArrayDevice[device_index];
	//						device_index++;
	//					}
	//				}
	//			}
	...
	//
	//// representation of haloed array
	////int haloed_index = 0;
	////for (int z = 0; z < haloed_depth; z++)
	////{
	////	for (int y =0 ; y < haloed_height; y++)
	...
	//// representation of haloed array after sending to device
	//int result_index = 0;
	//for (int z = 0; z < haloed_depth; z++)
	//{
	//	for (int y = 0; y < haloed_height; y++)
	...
	// printf("vec size: %d | haloed vec size: %d | original_ind: %d | haloed_index %d\n", vec.size(), haloed_vec.size(), original_index, haloed_index);
